...
// import { Upload, Play, Pause, CheckCircle, PlusCircle, XCircle, Trash2, Music, X } from 'lucide-react';
...
//     return a.id === b.id || (a.name === b.name && a.size === b.size);
//   };
...
//       : `${audioOnlyFiles.length} audio files uploaded successfully!`;
...
//         alert(`Selection limit is ${SELECTION_LIMIT} files.`);
//         return;
//       }
...
//                     {file.name}
//                   </h4>
//                   <p style={styles.fileSubtitle}>Web storage (session only)</p>
//                 </div>
...
//             <p style={styles.selectionHeaderText}>Tap a file to play from selection</p>
//             <button
...
// import React, { useState, useRef, useEffect } from 'react';
// import { Upload, Play, Pause, CheckCircle, PlusCircle, XCircle, Trash2, Music, X, Loader } from 'lucide-react';
...
//           showToast('Authentication failed. Some features may not work.', '#ef4444');
//         });
//       }
//     });
...
//   // Load audio files from Firebase Storage
//   const loadAudioFilesFromFirebase = async () => {
//     if (!user) return;
    
//     setIsLoading(true);
//     try {
//       const audioRef = ref(storage, `audio-files/${user.uid}`);
...
//       showToast('Error loading files from cloud storage', '#ef4444');
//     }
//     setIsLoading(false);
//   };
...
//     if (!user) {
//       showToast('Please wait for authentication...', '#f59e0b');
//       return;
//     }
...
//         showToast(`${file.name} uploaded successfully!`, '#10b981');
        
//       } catch (error) {
//         console.error('Error uploading file:', error);
//         showToast(`Error uploading ${file.name}: ${error.message}`, '#ef4444');
...
//       if (fileToDelete.isFirebaseFile && fileToDelete.firebaseRef) {
...
//   // Styles (same as before but with loading states)
//   const styles = {
//     container: {
//       minHeight: '100vh',
//       backgroundColor: '#f9fafb',
...
//                     {file.isFirebaseFile ? 'Stored in Firebase' : 'Web storage (session only)'}
//                   </p>
//                 </div>
...
//           >
//             <Upload size={20} />
//             <span>{user ? 'Upload Audio Files' : 'Authenticating...'}</span>
//           </button>
//         </div>
...
/*
 * Firebase Audio Player App
 * 
 * SETUP INSTRUCTIONS:
 * 1. Replace firebaseConfig with your actual Firebase config
 * 2. Enable Anonymous Authentication in Firebase Console:
 *    - Go to Authentication → Sign-in method
 *    - Enable "Anonymous" provider
 * 3. Set USE_FIREBASE_AUTH = true after enabling anonymous auth
 * 4. Update Firebase Storage rules to allow read/write:
 *    rules_version = '2';
...
//   // Firebase Authentication with configuration check
//   useEffect(() => {
//     if (!isFirebaseConfigured()) {
...
//               Currently running in <strong>local storage mode</strong>. Files will not persist across browser sessions.
//               <br />
...
//               <br />
//               3. Enable Anonymous Authentication and update Firebase Storage rules
//             </div>
//           </div>
//         )}
